        ...
        while (true) {
            try {
                ...
                $rs[] = $model->setTable('finance_' . $this->coin2, true)->save([
                    'userid' => $two['userid'],
                    'coinname' => $this->coin2,
                    'num_a' => $twoU[$this->coin2],
                    'num_b' => $twoU[$this->coin2 . 'd'],
                    'num' => round($twoU[$this->coin2] + $twoU[$this->coin2 . 'd'], $this->marketInfo['round']),
                    'fee' => $sell_fee,
                    'type' => 2,
                    'name' => 'tradelog',
                    'nameid' => $two['id'],
                    'remark' => '交易中心-成功卖出-市场' . $this->market,
                    'mum_a' => round($twoU[$this->coin2] + $sell_total, $this->marketInfo['round']),
                    'mum_b' => $twoU[$this->coin2 . 'd'],
                    'mum' => round(round($twoU[$this->coin2] + $sell_total, $this->marketInfo['round']) + $twoU[$this->coin2 . 'd'], $this->marketInfo['round']),
                    'move' => getFinanceHash(),
                    'addtime' => time(),
                    'status' => 1
                ]);
